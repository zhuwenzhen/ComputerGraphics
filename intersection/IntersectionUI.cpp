...
void IntersectionUI::changeShape(ShapesUI::ShapeType type)
{
	...
	case ShapesUI::SHAPE_SPHERE: {
									 shape = &sphere;
									 shape->computeTriangle(5);
	}break;

	case ShapesUI::SHAPE_CYLINDER:{
									  shape = &cylinder;
									  shape->computeTriangle(50, 2);
	} break;
	...
	case ShapesUI::SHAPE_CUBE: {
								   shape = &cube;
								   shape->computeTriangle(2);
	}break;
	case ShapesUI::SHAPE_TORUS: {
									shape = &torus;
									shape->computeTriangle(20);
	}break;
	}
}
...
void IntersectionUI::writeTest() const {
	...
	for (int i = seed; i < (seed + 1638); i++) {
		...
		Cube myCube;
		myCube.intersect(p, dir);
		cubeHr = myCube.hr;

		Cylinder myCylinder;
		myCylinder.intersect(p, dir);
		cylinderHr = myCylinder.hr;

		Cone myCone;
		myCone.intersect(p, dir);
		coneHr = myCone.hr;

		Sphere mySphere;
		mySphere.intersect(p, dir);
		sphereHr = mySphere.hr;
